...
<head>
<meta  charset=utf-8>
<title>Додати помічників гафера</title>
...
<h1 align="center" class="colorForAllText">Помічники гафера</h1></br>
...
<td>Обрати помічника гафера</td>
</tr></thead>
<form action="gafer_check.php" method="POST">
<?php
$mysqli = new mysqli("localhost","root","root","filmstudio");
$mysqli->query("SET NAMES 'utf8'");
...
$result_others=$mysqli->query("SELECT * FROM others WHERE `name_of_position` = 'помічник гафера' AND (others_id NOT IN (SELECT DISTINCT others_id FROM others_filmcrew WHERE number_of_film_crew
...
while ($stroka = mysqli_fetch_array($result_others)){
  ...
  $result_others_ratings = $mysqli->query("SELECT `rating` FROM `previous_movies_rating` WHERE `id_previous_movie_rating` IN (SELECT `id_previous_movie_rating` FROM  `others_previous_movies_ratings` WHERE `others_id` = $temp)");
    ...
    if($re[0] != ""){
      echo"<td>" ."<input type=\"checkbox\" checked class=\"form-control\" value = \"" . $stroka['others_id'] . "\" name=\"helper_id[]\" >";

    }else{
      echo"<td>" ."<input type=\"checkbox\" class=\"form-control\" value = \"" . $stroka['others_id'] . "\" name=\"helper_id[]\" >";
    }
