...
<nav class="navbar navbar-expand-lg navbar-light bg-light style=width=100%;">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    ...
    <form class=" my-2 my-lg-0">
      <label class=" mr-sm-2" >Звукрежисер</label>
    </form>
  </div>
</nav>
<div>
...
function res($result){
  ...
  while ($stroka = mysqli_fetch_array($result_filter)){
     $temp = $stroka['number_of_film_crew']; $result_movie = $mysqli->query("SELECT `name_of_movie` FROM `movie` WHERE `number_of_film_crew` = $temp");
    echo"<tr>";
    echo"<td name=\"number_of_film_crew\">" . $stroka['number_of_film_crew'] . "</td>"; $value = res($result_movie);
