...
<head>
<meta  charset=utf-8>
<title>Додати помічників художника по костюмах</title>
</head>
<style>
  #button_find{
    right: 20px;
...
<h1 align="center">Помічники художника по костюмах</h1>
...
<td>Обрати помічника художника по костюмах</td>
</tr>
<form action="hudozhnik_po_kostiumah_check.php" method="POST">
<?php
...
$result_others=$mysqli->query("SELECT * FROM others WHERE `name_of_position` = 'помічник художника по костюмах' AND others_id NOT IN (SELECT DISTINCT others_id FROM others_filmcrew WHERE number_of_film_crew
...
while ($stroka = mysqli_fetch_array($result_others)){
  ...
  $result_others_ratings = $mysqli->query("SELECT `rating` FROM `previous_movies_rating` WHERE `id_previous_movie_rating` IN (SELECT `id_previous_movie_rating` FROM  `others_previous_movies_ratings` WHERE `others_id` = $temp)");
    ...
    echo"<td>" ."<input type=\"checkbox\" value = \"" . $stroka['others_id'] . "\" name=\"others_id[]\" >";
    echo"</tr>";
   }
