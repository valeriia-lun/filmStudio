      ...
      //  $notFirst = false;
        if($films != NULL && $a[0] != ""){
          ...
          foreach ($films as $value) {
            ...
            echo $value;
            $quer = $quer . "`actor_id` IN (SELECT `actor_id` FROM `actors_filmCrew` WHERE `number_of_film_crew` IN (SELECT `number_of_film_crew` FROM `movie` WHERE `name_of_movie` = \"$value\"))";
            $isFirst = false;
          //  $notFirst = true;
            echo $quer;
          }
