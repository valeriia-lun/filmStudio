...
<head>
<meta  charset=utf-8>
<title>Знайти агента по акторах</title>
...
<select  class="select selectpicker  form-control">
  <option>Найдешевший агент по акторах</option>
  <option>Найдорожчий агент по акторах</option>
  <option>Агент по акторах з найбльшим стажем</option>
</select>
</div>
<form>
<div class="btn">
...
$result_films = $mysqli->query("SELECT `name_of_movie` FROM `movie` WHERE `number_of_film_crew` IN (SELECT `number_of_film_crew` FROM  `others_filmcrew` WHERE `others_id` = $temp)");
    ...
    echo"<td>" .  res($result_others_ratings) . "</td>"; echo"<td style=\"width:1px;white-space:nowrap;\">" .  res($result_films) . "</td>";
