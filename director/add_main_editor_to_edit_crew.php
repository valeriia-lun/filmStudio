...
<head>
<meta  charset=utf-8>
<title>Додати головного монтажера</title>
...
while ($stroka = mysqli_fetch_array($result_editors)){
  ...
  $result_editors_contacts_rel = $mysqli->query("SELECT `editor_relatives_phone_numbers` FROM `editor_contacts_of_relatives` WHERE `editor_id` IN (SELECT `editor_id` FROM  `editor` WHERE `editor_id` = $temp)");
    ...
    echo"<td>" ."<input type=\"checkbox\"  class=\"form-control\" onclick=\"onlyOne(this)\" value = \"" . $stroka['editor_id'] . "\" name=\"editor_id[]\" >";
    echo"</tr>";
   }
?>
...
<div class="btn">
<button  class ="button btn btn-danger" name="done">Додати головного монтажера</button></form>
  </div>
</html>
