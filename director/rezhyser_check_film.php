...
$mysqli->query("DELETE FROM `others_filmCrew` WHERE `number_of_film_crew` = $number_of_film_crew AND `others_id` IN(SELECT `others_id` FROM `Others` WHERE `name_of_position` = 'сценарист')");
$mysqli->query("DELETE FROM `others_filmCrew` WHERE `number_of_film_crew` = $number_of_film_crew AND `others_id` IN(SELECT `others_id` FROM `Others` WHERE `name_of_position` = 'лінійний продюсер')");
...
if ( $scenarist_id != '') {
    ...
    if ( $lin_produser_id != '') {
      foreach($lin_produser_id as $value){
        $result = $mysqli->query("INSERT INTO `others_filmcrew`(`number_of_film_crew`, `others_id`, `others_fee`) VALUES ('$number_of_film_crew', '$value','0')");
      if ($result) {
        echo "</br></br></br><h1 class=\"colorForAllText\">Лінійних продюсерів успішно додано до знімальної групи!</h1>";
       }
    else {
      echo "</br></br></br><h1 class=\"colorForAllText\">Здається, щось пішло не так, лінійних продюсерів не було додано..</h1>";
      echo "<a href=\"#\"  class=\"btn btn-danger\" onclick=\"history.back();return false;\">Назад</a>";
        }
      }
  }
